...
mojom::ModelAccess ParseAccess(const std::string& access_str) {
  if (access_str == "basic") {
...
void RemoteModelsFetcher::FetchModels(const std::string& url,
  ...
  auto result_callback = base::BindOnce(
      [](base::WeakPtr<RemoteModelsFetcher> fetcher,
         FetchModelsCallback callback,
        ...
        fetcher->OnFetchComplete(std::move(callback), std::move(result));
      },
...
std::vector<mojom::ModelPtr> RemoteModelsFetcher::ParseModelsFromJSON(
  ...
  for (const auto& model_value : *models_list) {
    ...
    leo_opts->description = *options_dict->FindString(kDescriptionField);
    ...
    leo_opts->access =
        access ? ParseAccess(*access) : mojom::ModelAccess::BASIC;

    // Server returns calculated values or placeholder (1)
    // Use server's values as-is, with fallback defaults only if field is missing
    int max_content_length =
        options_dict->FindInt(kMaxAssociatedContentLengthField).value_or(64000);
    int warning_limit =
        options_dict->FindInt(kLongConversationWarningCharacterLimitField)
            .value_or(9700);

    // If placeholder value (1), use reasonable defaults based on access tier
    // Server returns 1 when context_window data is unavailable
    if (max_content_length == 1) {
      // Premium models typically have larger context windows
      max_content_length =
          (leo_opts->access == mojom::ModelAccess::PREMIUM) ? 90000 : 32000;
      VLOG(1) << "Using default max_associated_content_length for "
              << model->key << ": " << max_content_length;
    }
    ...
    if (warning_limit == 1) {
      ...
      VLOG(1) << "Using default long_conversation_warning_character_limit for "
              << model->key << ": " << warning_limit;
    }

    leo_opts->max_associated_content_length = max_content_length;
    leo_opts->long_conversation_warning_character_limit = warning_limit;
...
bool RemoteModelsFetcher::ValidateModel(
  ...
  // Validate numeric fields are positive (allow placeholder value of 1)
  std::optional<int> max_content =
      options->FindInt(kMaxAssociatedContentLengthField);
  if (max_content.has_value() && *max_content < 1) {
  ...
  if (warning_limit.has_value() && *warning_limit < 1) {
...
void RemoteModelsFetcher::OnFetchComplete(
  ...
  cached_endpoint_url_ = result.final_url().spec();
