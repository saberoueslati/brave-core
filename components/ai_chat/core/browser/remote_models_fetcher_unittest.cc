...
class RemoteModelsFetcherTest : public testing::Test {
 ...
 protected:
  void SimulateSuccessfulFetch(const std::string& json_response) {
    test_url_loader_factory_.AddResponse(kTestEndpoint, json_response);
  }

  void SimulateHTTPError(int http_code) {
    test_url_loader_factory_.AddResponse(
        kTestEndpoint, "", static_cast<net::HttpStatusCode>(http_code));
  }
...
TEST_F(RemoteModelsFetcherTest, SavesToPrefs) {
  ...
  const base::Value::List* models = cache.FindList("models");
  ASSERT_TRUE(models);
...
TEST_F(RemoteModelsFetcherTest, AllowsHTTPForLocalhost) {
  ...
  test_url_loader_factory_.AddResponse(kLocalhostEndpoint, kValidModelsJSON);
